 ...
 * @author dkzwm
 */
public class SmoothRefreshLayout extends ViewGroup implements OnGestureListener,
        NestedScrollingChild2, NestedScrollingParent2, ViewTreeObserver.OnScrollChangedListener {
    //status
     ...
     * @param interpolator Scroller interpolator
     */
    public void setOverScrollInterpolator(@NonNull Interpolator interpolator) {
        mOverScrollInterpolator = interpolator;
        if (mScrollChecker.isFling() || mScrollChecker.isPreFling()) {
            mScrollChecker.updateInterpolator(interpolator);
        }
    }
    ...
    public void onScrollChanged() {
        if (mNeedFilterScrollEvent || mNestedScrollInProgress) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (action == MotionEvent.ACTION_UP) {
                    final int deltaX = (int) (ev.getX() - mSendDownPoint[0]);
                    final int deltaY = (int) (ev.getY() - mSendDownPoint[1]);
                    int distance = (deltaX * deltaX) + (deltaY * deltaY);
                    if (mHasSendDownEvent && distance <= mTouchSlop * mTouchSlop
                            && mSendDownEventTime - ev.getEventTime()
    ...
    protected void sendDownEvent(MotionEvent event) {
        ...
        mSendDownEventTime = last.getEventTime();
        mSendDownPoint[0] = last.getX();
        mSendDownPoint[1] = last.getY();
        super.dispatchTouchEvent(ev);
        ev.recycle();
    }
    ...
    protected void notifyUIRefreshComplete(boolean useScroll) {
        ...
        if (mNeedNotifyRefreshComplete) {
            ...
            mNeedNotifyRefreshComplete = false;
        } else if (mDelayedRefreshComplete && mRefreshListener != null) {
            mRefreshListener.onRefreshComplete(mIsLastRefreshSuccessful);
        }
    ...
    protected void tryToRemoveDelayToDispatchNestedFling() {
        if (mDelayToDispatchNestedFling != null) {
            ...
            mDelayToDispatchNestedFling = null;
        }
    }
    ...
    protected boolean canPerformRefresh() {
        return mMode == Constants.MODE_DEFAULT && !isMovingContent();
    }
    ...
    protected void tryToDispatchNestedFling(int velocity) {
        tryToRemoveDelayToDispatchNestedFling();
        mDelayToDispatchNestedFling = new DelayToDispatchNestedFling(this, velocity);
    ...
    private static class DelayToDispatchNestedFling implements Runnable {
        ...
        private DelayToDispatchNestedFling(SmoothRefreshLayout layout, int velocity) {
            mLayoutWeakRf = new WeakReference<>(layout);
            mVelocity = velocity;
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        void tryToPreFling(float v) {
            ...
            final int finalY;
            if (SmoothRefreshLayout.this.isMovingHeader() && mVelocity > 0) {
                int maxDistance = (int) SmoothRefreshLayout.this.mIndicator
                        .getCanMoveTheMaxDistanceOfHeader();
                int distance = maxDistance - SmoothRefreshLayout.this.mIndicator.getCurrentPos();
                finalY = maxDistance > 0 ? Math.min(distance, mScroller.getFinalY()) :
                        mScroller.getFinalY();
            } else if (SmoothRefreshLayout.this.isMovingFooter() && mVelocity < 0) {
                int maxDistance = (int) SmoothRefreshLayout.this.mIndicator
            ...
            mDuration = mScroller.getDuration();
            if (SmoothRefreshLayout.sDebug)
                SRLog.d(SmoothRefreshLayout.this.TAG, "ScrollChecker: tryToPreFling(): v: %s, finalY: %s," +
                        " duration: %s", v, finalY, mDuration);
            mScroller.startScroll(0, 0, 0, finalY, mDuration);
            ViewCompat.postOnAnimation(SmoothRefreshLayout.this, this);
        }
        ...
        void tryToScrollTo(int to, int duration) {
            ...
            if (to > SmoothRefreshLayout.this.mIndicator.getCurrentPos()) {
                updateInterpolator(SmoothRefreshLayout.this.mSpringInterpolator);
            } else if (to < SmoothRefreshLayout.this.mIndicator.getCurrentPos()) {
                updateInterpolator(SmoothRefreshLayout.this.mSpringBackInterpolator);
            } else {
                return;
            }
            if (SmoothRefreshLayout.sDebug)
        ...
        void destroy() {
            if (SmoothRefreshLayout.sDebug)
                SRLog.d(SmoothRefreshLayout.this.TAG, "ScrollChecker: destroy()");
            mMode = MODE_NONE;
            mIsScrolling = false;
