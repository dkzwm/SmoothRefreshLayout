...
public class SmoothRefreshLayout extends ViewGroup implements OnGestureListener, NestedScrollingChild,
    ...
    public SmoothRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDefaultSpringInterpolator = new DecelerateInterpolator(.85f);
        //Nested scrolling
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (dy == 0) {
            mIndicator.onFingerMove(mIndicator.getLastMovePoint()[0] - dx,
                    mIndicator.getLastMovePoint()[1]);
            updateXPos();
        } else if (isMovingFooter() && mIndicator.hasLeftStartPosition() && canChildScrollDown()) {
            mScrollChecker.tryToScrollTo(IIndicator.DEFAULT_START_POS, 0);
            consumed[1] = dy;
        }
        onNestedPreScroll(dx, dy, consumed);
    ...
    protected boolean processDispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (canMoveUp) {
                    if (isLoadingMore() || isDisabledRefresh() || (!canHeaderMoveDown && movingDown))
                        return super.dispatchTouchEvent(ev);
                    moveHeaderPos(offsetY);
                    return true;
                }
                if (mStatus == SR_STATUS_COMPLETE) {
                    ...
                    if (isRefreshing() || isDisabledLoadMore() || (!canFooterMoveUp && !movingDown))
                        return super.dispatchTouchEvent(ev);
                    moveFooterPos(offsetY);
                    return true;
                }
    ...
    protected void updateYPos(int change) {
        ...
        if (!mOverScrollChecker.mScrolling && mStatus == SR_STATUS_PREPARE) {
            ...
            if (mIndicator.hasTouched() && !isAutoRefresh() && isEnabledPullToRefresh()
                    && ((isMovingHeader && mIndicator.crossRefreshLineFromTopToBottom())
            ...
            if (!isRefreshing() && !isLoadingMore() && isPerformAutoRefreshButLater()
                    && ((isMovingHeader && mIndicator.hasJustReachedHeaderHeightFromTopToBottom())
        ...
        if (mNeedScrollCompat || needRequestLayout || (!mOverScrollChecker.mScrolling
     ...
     * @return Is
     */
    private boolean isNeedScrollBackToTop() {
    ...
    private class OverScrollChecker implements Runnable {
        ...
        private OverScrollChecker(SmoothRefreshLayout layout) {
            ...
            mScroller = new Scroller(layout.getContext(), SmoothRefreshLayout
                    .sOverScrollInterpolator, false);
        }

        private void fling(float vy) {
            if (SmoothRefreshLayout.sDebug) {
                SRLog.d(SmoothRefreshLayout.TAG, "OverScrollChecker: fling(): vy: %s", vy);
            }
            destroy();
            mFling = true;
        ...
        private void setClamped(boolean clamped) {
            mClamped = clamped;
            if (SmoothRefreshLayout.sDebug) {
                SRLog.d(SmoothRefreshLayout.TAG, "OverScrollChecker: setClamped(): clamped: %s",
                        clamped);
            }
        }
        ...
        public void run() {
            ...
            if (!mScroller.isFinished()) {
                ...
                if (currY > 0 && !SmoothRefreshLayout.this.canChildScrollUp()
                    ...
                    int to = Math.round(mScroller.getFinalY() - currY);
                    mDuration = Math.round((mScroller.getDuration() - mScroller.timePassed()) *
                            SmoothRefreshLayout.this.mOverScrollDurationRatio);
                    final float maxOverScrollDuration = SmoothRefreshLayout.this.mMaxOverScrollDuration;
                    final int optimizedDistance;
                    if (mDuration > maxOverScrollDuration) {
                        mDuration = Math.round(maxOverScrollDuration * maxOverScrollDuration * .5f
                                / mDuration);
                        optimizedDistance = mMaxDistance;
                    } else {
                        if (mDuration > SmoothRefreshLayout.this.mMinOverScrollDuration) {
                            optimizedDistance = Math.round(mMaxDistance * .55f /
                    ...
                    final int headerHeight = SmoothRefreshLayout.this.getHeaderHeight();
                    final int maxDistance = headerHeight > 0
                            ? Math.min(headerHeight, optimizedDistance) : optimizedDistance;
                    ...
                    reset();
                    return;
                } else if (currY < 0 && !SmoothRefreshLayout.this.canChildScrollDown()
                        && !SmoothRefreshLayout.this.isRefreshing()
                        && !(SmoothRefreshLayout.this.isEnabledPinRefreshViewWhileLoading()
                        && SmoothRefreshLayout.this.isLoadingMore())) {
                    int to = Math.round(currY - mScroller.getFinalY());
                    mDuration = Math.round((mScroller.getDuration() - mScroller.timePassed()) *
                            SmoothRefreshLayout.this.mOverScrollDurationRatio);
                    final float maxOverScrollDuration = SmoothRefreshLayout.this.mMaxOverScrollDuration;
                    final int optimizedDistance;
                    ...
                    final int footerHeight = SmoothRefreshLayout.this.getFooterHeight();
                    final int maxDistance = footerHeight > 0
                            ? Math.min(footerHeight, optimizedDistance) : optimizedDistance;
                    to = to > maxDistance ? maxDistance : to;
